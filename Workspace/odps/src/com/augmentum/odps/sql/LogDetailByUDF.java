...
public class LogDetailByUDF {
	...
	public static void main(String[] args) throws Exception {
		...
		LogDetailByUDF logDetail = new LogDetailByUDF();
        
		// 1.Creates a temporary table 'tmp' to store json-parsed and xml data(drop table 'tmp' if exists).
		// 2.Inserts data into table 'tmp' according to table 'wx_accesslogs'.
		// 3.Gets log's count.
		String countSql = "select count(1) from tmp where sls_time >= "
		...
		while (beginIndex < totalCount) {
			...
			if(info.length > 1){
				for (int i = 1; i < info.length; i++) {
					...
					// 6.Inserts data into table 'wx_accesslogs_detail' one by one when finishes parsing.
					String insertSql = "";
					if (data.size() > 0) {
